...
// Ensure Fluent Forms assets are loaded for the contact form
add_action('wp_footer', function() {
    if (!function_exists('wpFluentForm')) return;

    $plugin_url = plugins_url('', FLUENTFORM_PLUGIN);

    // Fluent Forms required JS variables
    ?>
    <script>
    if (typeof fluentFormVars === 'undefined') {
        var fluentFormVars = {
            ajaxUrl: '<?php echo admin_url('admin-ajax.php'); ?>',
            nonce: '<?php echo wp_create_nonce('fluentform-public-nonce'); ?>',
            force_init: true
        };
    }
    </script>
    <script src="<?php echo $plugin_url; ?>/assets/js/form-submission.js?ver=<?php echo FLUENTFORM_VERSION; ?>"></script>
    <?php
}, 99);
...
(function() {
    ...
    function prefillFluentForm() {
        ...
        // Find hidden fields in Fluent Form
        const modal = document.getElementById('contact-modal');
        if (!modal) return;

        // Prefill LO ID field
        const idFields = modal.querySelectorAll('input[name*="frs_loan_officer_id"], input[name*="loan_officer_id"]');
        idFields.forEach(f => f.value = loId);

        // Prefill LO email field
        const emailFields = modal.querySelectorAll('input[name*="frs_loan_officer_email"], input[name*="loan_officer_email"]');
        emailFields.forEach(f => f.value = loEmail);

        // Prefill LO name field
        const nameFields = modal.querySelectorAll('input[name*="frs_loan_officer_name"], input[name*="loan_officer_name"]');
        nameFields.forEach(f => f.value = loName);
