...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // State coordinates (approximate % positions on a USA map layout)
    // Adjusted to fit within hero section bounds
    const stateCoords = {
        // West Coast
        'WA': { left: 12, top: 12 },
        'OR': { left: 11, top: 22 },
    ...
    function populateHeroAvatars() {
        // Only use profiles with real headshot images and service areas
        const withHeadshots = profiles.filter(p =>
            p.headshot_url && p.headshot_url.trim() !== '' &&
            p.service_areas && Array.isArray(p.service_areas) && p.service_areas.length > 0
        );
        heroAvatars.innerHTML = '';

        // Track positions used to add jitter for overlapping states
        const stateOffsets = {};

        // Shuffle to get variety
        const shuffled = [...withHeadshots].sort(() => Math.random() - 0.5);

        // Limit to reasonable number
        const maxAvatars = 35;
        const toShow = shuffled.slice(0, maxAvatars);

        toShow.forEach((p, i) => {
            // Get primary state (first service area, normalized)
            const primaryState = normalizeState(p.service_areas[0]);
            const coords = stateCoords[primaryState];

            if (!coords) return; // Skip if state not found

            // Add jitter if multiple LOs in same state
            if (!stateOffsets[primaryState]) stateOffsets[primaryState] = 0;
            const offset = stateOffsets[primaryState];
            stateOffsets[primaryState]++;

            // Calculate position with jitter (spread out multiple LOs in same state)
            const jitterX = (offset % 3) * 4 - 4; // -4, 0, 4
            const jitterY = Math.floor(offset / 3) * 4;
            const left = Math.max(2, Math.min(96, coords.left + jitterX));
            const top = Math.max(2, Math.min(96, coords.top + jitterY));

            // Vary size slightly
            const size = 48 + (i % 5) * 6; // 48-72px

            const div = document.createElement('div');
            div.className = 'frs-hero__avatar';
            div.dataset.state = primaryState;

            // Add position-based classes for responsive layouts
            const isLeftEdge = left <= 20;
            const isRightEdge = left >= 80;
            const isTopArea = top <= 25;
            const isBottomArea = top >= 75;
            const isMiddleVertical = top > 25 && top < 75;
            ...
            div.innerHTML = `<img src="${p.headshot_url}" alt="${firstName} ${lastName}" title="${firstName} ${lastName} - ${primaryState}" loading="lazy">`;
